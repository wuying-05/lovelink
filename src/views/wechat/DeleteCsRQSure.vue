  ...
  <div class="deleteuser-wrap">
    <div v-if="canDelete">
      <el-row v-loading="dialog_loading">
        <div class="title"><span>确认删除此客服二维码？</span></div>
...
export default {
  ...
  created() {
    ...
    if (this.$route.query.code && this.$route.query.state >= 0) {
      ...
      let state = 0;
      const redirect_uri =
        `https://lovelink.net.cn/delete_csrq?rqid=` + this.rqid;
  ...
  methods: {
    ...
    getDeleteInfo(code, rqid) {
      ...
      this.$http
        .post("http://wechat.lovelink.net.cn/api/delete_cs_rq/sure", datas)
        .then(({ data }) => {
          if (data.state === "ok") {
    ...
    deluser(t_code, t_openid, t_rqid) {
      ...
      this.$http
        .post("http://wechat.lovelink.net.cn/api/delete_cs_rq/do", datas)
        .then(({ data }) => {
