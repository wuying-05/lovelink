      ...
      <table style="width: 100%; height: 100%">
        ...
        <tbody>
          <td>
            <ul>
              <li>
                年龄:<span v-if="!HeToMe.age_match" style="color: red">
                  不接受</span
                ><span v-else style="color: blue"> 接受</span>
              </li>
              <li>
                身高:<span v-if="!HeToMe.height_match" style="color: red">
                  不接受</span
                ><span v-else style="color: blue"> 接受</span>
              </li>
              <li>
                婚姻状况:<span v-if="!HeToMe.marriage_match" style="color: red">
                  不接受</span
                ><span v-else style="color: blue"> 接受</span>
              </li>
              <li>
                学历:<span v-if="!HeToMe.degree_match" style="color: red">
                  不接受</span
                ><span v-else style="color: blue"> 接受</span>
              </li>
              <li>
                是否带小孩:<span
                  v-if="!HeToMe.children_match"
                  style="color: red"
                >
                  不接受</span
                ><span v-else style="color: blue"> 接受</span>
              ...
              <li>
                购房情况:<span v-if="!HeToMe.house_match" style="color: red">
                  不接受</span
                ><span v-else style="color: blue"> 接受</span>
              </li>
              <li>
                是否有照片进行匹配:<span
                  v-if="!HeToMe.picture_match"
                  style="color: red"
                >
                  不接受</span
                ><span v-else style="color: blue"> 接受</span>
          ...
          <td>
            <ul>
              <li>
                年龄:<span v-if="!MeToHe.age_match" style="color: red">
                  不接受</span
                ><span v-else style="color: blue"> 接受</span>
              </li>
              <li>
                身高:<span v-if="!MeToHe.height_match" style="color: red">
                  不接受</span
                ><span v-else style="color: blue"> 接受</span>
              </li>
              <li>
                婚姻状况:<span v-if="!MeToHe.marriage_match" style="color: red">
                  不接受</span
                ><span v-else style="color: blue"> 接受</span>
              </li>
              <li>
                学历:<span v-if="!MeToHe.degree_match" style="color: red">
                  不接受</span
                ><span v-else style="color: blue"> 接受</span>
              </li>
              <li>
                是否带小孩:<span
                  v-if="!MeToHe.children_match"
                  style="color: red"
                >
                  不接受</span
                ><span v-else style="color: blue"> 接受</span>
              ...
              <li>
                购房情况:<span v-if="!MeToHe.house_match" style="color: red">
                  不接受</span
                ><span v-else style="color: blue"> 接受</span>
              </li>
              <li>
                是否有照片进行匹配:<span
                  v-if="!MeToHe.picture_match"
                  style="color: red"
                >
                  不接受</span
                ><span v-else style="color: blue"> 接受</span>
                ...
                <el-col :lg="6" :md="6" :sm="22" style="margin: 20px">
                  ...
                  <span>
                    <el-button
                      ...
                      "
                      v-if="sharedState.user_id !== userData.id"
                      >发消息</el-button
                    >
...
export default {
  ...
  methods: {
    async getHeToMeList(hisId) {
      //默认获取 他对我 的匹配度数据
      this.loading = true;
      const self = this;
      const path = `/v1/match/${hisId}`; /* "/v1/match/"+hisId ,参数hisId 为用来和自己匹配的id */
      this.$axios
        .get(path)
        .then((response) => {
          console.log("访问成功,response:", response);
          const { data: res } = response;
          console.log("访问成功,res:", res);
          if(typeof(res)==="object"){
            this.HeToMe = res; //赋值给data中的HeToMe
          }else{
            console.log("error : 接收数据错误！");
          }
        })
        .catch((error) => {
          console.log(error);
    ...
    async getMeToHeList(hisId) {
      //默认获取 他对我 的匹配度数据
      this.loading = true;
      const self = this;
      const path = `/v1/match_myself/${hisId}`; /* "/v1/match_myself/"+hisId ,参数hisId 为用来和自己匹配的id*/
      this.$axios
        .get(path)
        .then((response) => {
          console.log("访问成功,response:", response);
          ...
          console.log("访问成功,res:", res);
          if(typeof(res)==="object"){
            this.MeToHe = res; //赋值给data中的MeToHe
          }else{
            console.log("error : 接收数据错误！");
          }
        })
    ...
    showwechatworkgroup() {
      this.getHeToMeList(this.userData.id);
      this.getMeToHeList(this.userData.id);
    ...
    getContactAbility(id) {
      ...
      this.$axios
        ...
        .then((response) => {
          console.log(
            "response: response:[esponse.data.visable]" + esponse.data.visable
          );
