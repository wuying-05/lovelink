<template>
  <div class="blind" v-loading="loading">
    ...
    <el-dialog title="删除相亲群二维码操作" :custom-class="dialog_rqdelete"  :visible.sync="dialog_rqdelete" width="300px" >
      <DeleteGroupChat :rqid="dialog_rqdelete_id"  v-if="dialog_rqdelete"></DeleteGroupChat>
      </el-dialog>
...
import DeleteGroupChat from '@/views/DeleteGroupChat';
...
export default {
  ...
  data() {
    return {
      ...
      activeName: "first",
      introduction:
        "客户通过扫描公众号菜单的二维码，即可获取入群方式，进入企业的客户群。",
      searchText: "",
      tabsLabel: ["客户群二维码"],
      areaValue: [],
      areaList: [],
  ...
  methods: {
    ...
    newList() {
      ...
      this.add = true;
      this.$nextTick(() => {
        this.$refs.AddAreaList.getSelect();
      });
    },
    searchList() {
      if (this.searchText) {
    ...
    saveData(e) {
      ...
      this.$nextTick(() => {
        this.type = 1;
        this.$refs.AddAreaList.modify(e.row);
      });
    },
    loadingFlag(flag) {
      this.loading = flag;
    ...
    deletData(e) {
      ...
      // .then(()=>{
      //   this.loading = true
      //   this.$request.post('v1/admin/wxgroup_rqcode_manager/delete',{
      //     type: 'location',
      //     id: e.row.id
      //   })
      //   .then(res=>{
    ...
    getList(key) {
      ...
      this.loading = true;
      this.$request
        .post("v1/admin/wxgroup_rqcode_manager/page", data)
        .then((res) => {
          this.loading = false;
