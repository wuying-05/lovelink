    ...
    <el-row>
      ...
      >
        ...
        <div class="lay-item-text">
          ...
          <div
            style="color: #333333; padding: 0; font-size: 13px"
            v-html="value.content"
          ></div>
          <div style="color: #666666; margin-bottom: 10px; font-size: 11px">
            {{ value.tm }}
          </div>
        </div>

        <div @click="showMessage(value.content, value.id)" class="btn-see">
          查看
          <!-- 1、未读  2、已读 -->
          <div class="point" v-if="value.see_type === 1"></div>
        </div>
...
export default {
  ...
  methods: {
    ...
    showMessage(mes, id) {
      ...
      this.$axios
        .post("/users/check/info/" + this.sharedState.user_id, params)
        .then((response) => {
